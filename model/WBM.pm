...
my $version =		'23.11.0';	# Version. Note- update version variable below
#
#######################################################################
		...
		### Reads a data layer by given date
{
  ...
  if ($spool_dir) {
    ...
    if (-e $file) {
      ...
      if (!$dataSet->{spDelta} || $date =~ m/-01$/) {		### NB - Hourly datasets have spDelta set to False
...
sub spoolFile_read
{
  ...
  for (my $day=$date_prev[2]+1; $day<=$date[2]-1; $day++) {	# $date[2]-1 is to avoid reading same file again
    $FILE	=~ s/\d{2}\.dat/sprintf("%02d",$day).".dat"/e;
   ($data,$idx) = readflex($FILE);
    $DATA->flat->index($idx) += $data;
  }
